ここに GPI.exe の実行結果（以下「GPI」）を貼っておくと、そのクライアント用スクリプトが作成される可能性が高まるかもしれません。
...
```
なお、GPI は `{{{` と `}}}` で囲って貼っていただけると上の例のように割と見やすく表示されます。<br>
また、「BRAVIA KDL-40J5 の GPI は、BRAVIA KDL-32F1 と同じです。」というような情報も地味ながら貴重な情報なのではないかと思います。